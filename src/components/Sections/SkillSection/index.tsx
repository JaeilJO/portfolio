...
function SkillSection() {
  ...
  const rightArrowStatus =
    currentSkill === Skills.length - 1
      ? arrowStatus.unavailable
      : arrowStatus.available;
  const leftArrowStatus =
    currentSkill === 0 ? arrowStatus.unavailable : arrowStatus.available;

  const skillSomething =
    "absolute w-[.3em] h-[.3em] bg-neutral-50  dark:bg-neutral-900 border-2 border-neutral-900 dark:border-neutral-50";
      ...
      <div className="relative w-full h-full lg:text-6xl">
        <div className="absolute w-[7em] h-[7em] top-2/4 left-2/4 translate-x-[-50%] translate-y-[-50%] bg-neutral-50  dark:bg-neutral-900 border-2 border-neutral-900 dark:border-neutral-50 z-10 duration-200 ">
          ...
          <div className="absolute z-0 w-full h-full overflow-hidden">
            <div
              className={`absolute flex h-full text-4xl lg:text-6xl duration-200 ${setTranslateValue}`}
            >
              {Skills.map((skill) => (
                <div
                  className={`w-[7em] h-full flex justify-center items-center`}
                  key={skill.key}
                >
